...
def process_image(image, time):
    ...
    vertices = np.array([[(0, max_y), (600, top_y), (700, top_y), (max_x, max_y)]], dtype=np.int32)
    ...
    left_a, right_a, image_after_hough = hough_lines(image_mask, rho, theta, threshold, min_line_length, max_line_gap)
    ...
    if not np.isclose(left_slope, left_a, atol=0.2) or not np.isclose(right_slope, right_a, atol=0.2):
        plt.imshow(image_mask, cmap='gray')
        mpimg.imsave('test_images_output_video/' + input_file_name + "_t%0.2f_l%0.1f_r%0.1f.jpg" % (time, left_a, right_a), image_weighted)
